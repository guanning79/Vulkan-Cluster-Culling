...
const std::string MODEL_PATH = "models/new_building_01.obj";
...
public:
    ...
    static void MouseScrollProcess(GLFWwindow* _win, double _xoff, double _yoff)
    {
        printf("[TestLog]: Mouse scrolled for %f\n", (float)_yoff);
        sAppInstance->camMoveDistance += (float)(-_yoff);
    }

    static void KeyEventProcess(GLFWwindow* window, int key, int scancode, int action, int mods)
    {
        switch (key)
        {
        case GLFW_KEY_F:
    ...
    typedef void (*HMouseScrollCallback) (GLFWwindow*, double, double);
    HMouseScrollCallback    mouseScrollFunc = nullptr;
...
private:
    ...
    void createGraphicsPipelineFamily() {
        ...
        pipelineStat.vsEntry = "main";
        pipelineStat.fsName = "shaders/frag.spv";
        pipelineStat.fsEntry = "main";
        pipelineStat.descSetLayoutKey = pipelineLayoutKey;
        pipelineStat.renderPass = renderPass;
        pipelineStat.sampleCountFlags = msaaSamples;
        pipelineStat.swapChainExtent = swapChainExtent;
...
VkSampleCountFlagBits getMaxUsableSampleCount() {
    ...
    void updateRenderPass(size_t _imageIdx)
    {
        size_t commandBufferIdx = _imageIdx;
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        ...
        renderPassInfo.framebuffer = swapChainFramebuffers[_imageIdx];
        ...
        if (vkEndCommandBuffer(commandBuffers[commandBufferIdx]) != VK_SUCCESS) {
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        {
            glm::vec3 camDirection = cameraPosition - glm::vec3(0.0f, 0.0f, 0.0f);
            float len = glm::length(camDirection);
            if (len > 0.00001f)
            {
                camDirection /= len;
                cameraPosition = camDirection * (camMoveDistance + len);
                camMoveDistance = 0.0f;
            }
        }
...
int main() {
    ...
    HelloTriangleApplication::sAppInstance = &app;
    app.mouseScrollFunc = &app.MouseScrollProcess;
    app.keyEventFunc = &app.KeyEventProcess;
